...
def parse_model(d, ch, verbose=True):
    ...
    # 2. 模块分类注册 (包含你的自定义模块)
    # base_modules: 需要进行宽度缩放 (width_multiple) 的模块
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        m_.np = sum(x.numel() for x in m_.parameters())  # 参数量
        m_.i, m_.f, m_.type = i, f, t  # 绑定索引、来源、类型
