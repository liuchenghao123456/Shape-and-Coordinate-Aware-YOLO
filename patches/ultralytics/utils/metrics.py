...
def bbox_iou(
    ...
    if WIoU:
        ...
        # R_WIoU = exp(dist^2 / scale^2)
        # 使用 .detach() 是 WIoU 的核心，防止闭合框大小参与梯度
        dist_sq = (x1 - x2).pow(2) + (y1 - y2).pow(2)
        scale_sq = (cw.pow(2) + ch.pow(2)).detach() + eps
        r_wiou = torch.exp(dist_sq / scale_sq)

        # 注意：为了配合 loss.py 里的 v3 动态逻辑，这里返回两个值
        return r_wiou, iou
